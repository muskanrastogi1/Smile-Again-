...
const adminAadhar = require("../model/model.js").adminAadhar;
const admin = require("../model/model.js").admin;
    ...
    if (!helpers.emailValidate(request.body.email)) {
        ...
        .then(data => {
            if (data[0] != undefined || data[0] != null || data[0] != []) {
                ...
                console.log("The given details didn't matched our Aadhar Database try again!!");
                response.send("The given details didn't matched our Aadhar Database try again!!");
            }
        })
        .catch( err => {
            console.log("There was some error verifying the data from the Aadhar Database");
            response.send("There was some error verifying the data from the Aadhar Database")
        }) 
...
router.post("/admin/login", (request, response) => {
    ...
    .catch( err => {
        console.log("There was some error verifying the details")
        response.send("There was some error verifying the details")
    }) 
})
